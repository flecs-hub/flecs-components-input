...
ECS_CTOR(EcsClassList, ptr, {
    ...
    ecs_vec_init_t(NULL, &ptr->added_by, ecs_entity_t, 0);
})
...
ECS_DTOR(EcsClassList, ptr, {
    ...
    ecs_vec_fini_t(NULL, &ptr->added_by, ecs_entity_t);
})
    ...
    ecs_entity_t by)
{
    for (int32_t i = 0; i < count; i ++) {
        ...
        if (arr[i].added > added) {
            ...
            ecs_os_memmove_n(
                &arr[i], &arr[i + 1], ecs_classlist_added_by_t, count - i);
            arr[i].added = added;
            arr[i].by = by;
            return count + 1;
        }
    ...
    for (int32_t i = 0; i < it->count; i ++) {
        ...
        for (int32_t c = 0; c < class_count; c ++) {
            ...
            for (int32_t t = 0; t < type->count; t ++) {
                ...
                if (ecs_has_id(world, e, EcsDontInherit)) {
                    continue;
                }
            ...
            /* Remove ids that were added by classlist but are no longer */
            int32_t t_old, t_new = 0;
            for (t_old = 0; t_old < ecs_vec_count(added_by); t_old ++) {
                ecs_id_t new_id = ecs_vec_get_t(added_by, ecs_classlist_added_by_t, t_new)->added;
                ecs_id_t old_id = local_added_by[t_old].added;
            ...
            for (; t_old < ecs_vec_count(added_by); t_old ++) {
                ecs_id_t old_id = local_added_by[t_old].added;
                ecs_remove_id(world, tgt, old_id);
            }
            ...
            for (int32_t t = 0; t < added_by_count; t ++) {
                ...
                if (!ti) {
                    ...
                    void *dst = ecs_get_mut_id(world, tgt, id);
                    void *src = ecs_get_mut_id(world, by, id);
                    if (ti->hooks.copy) {
